...
public class LobbyManager : MonoBehaviour
{
    ...
    {
        Debug.Log(
            "Executing CreatePlayerListItems. This many players to create: "
                + Game.GamePlayers.Count.ToString()
        );
        foreach (GamePlayer player in Game.GamePlayers)
        {
            Debug.Log(
                "CreatePlayerListItems: Creating playerlistitem for player: " + player.playerName
            );
            GameObject newPlayerListItem = Instantiate(PlayerListItemPrefab) as GameObject;
            PlayerListItem newPlayerListItemScript =
                newPlayerListItem.GetComponent<PlayerListItem>();
    ...
    private void CreateNewPlayerListItems()
    {
        ...
        foreach (GamePlayer player in Game.GamePlayers)
        {
            ...
            {
                Debug.Log(
                    "CreateNewPlayerListItems: Player not found in playerListItems: "
                        + player.playerName
                );
                GameObject newPlayerListItem = Instantiate(PlayerListItemPrefab) as GameObject;
                PlayerListItem newPlayerListItemScript =
    ...
    private void RemovePlayerListItems()
    {
        ...
        foreach (PlayerListItem playerListItem in playerListItems)
        {
            ...
            {
                Debug.Log(
                    "RemovePlayerListItems: player list item fro connection id: "
                        + playerListItem.ConnectionId.ToString()
                        + " does not exist in the game players list"
                );
    ...
    void CheckIfAllPlayersAreReady()
    {
        ...
        foreach (GamePlayer player in Game.GamePlayers)
        {
            ...
            {
                Debug.Log(
                    "CheckIfAllPlayersAreReady: Not all players are ready. Waiting for: "
                        + player.playerName
                );
